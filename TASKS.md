...
- [x] **Dry-run mode** — `DRY_RUN=true` env var added. `post_comment()` and `apply_labels()` in `core-runner.sh` no-op when dry-run. `open_github_pr.sh` exits before push/PR create/label mutations and prints what would have happened. Added to `.env.example` and `docker-compose.yml`.
- [x] **Fixture issue** — `tests/fixtures/sample-issue.json` contains a realistic DT contact field issue. `tests/fixtures/sample-repo/` is a minimal plugin structure. `tests/run_fixture_test.sh` exercises blueprint validation and secret scanner without needing live credentials.
- [x] **Stage isolation testing** — `tests/mock-env.sh` exports all required env vars with safe defaults (`DRY_RUN=true`, `REPO_DIR=tests/fixtures/sample-repo`). `tests/run_stage.sh <stage-key>` sources mock-env and runs any stage by short key (`01`, `02`, `03/01`, `04`, etc.).
- [x] **Security vulnerabilities** — Four threats identified and mitigated: (1) Prompt injection via issue content — explicit security boundary added to both planning and execute prompts; model instructed to write BLOCKED/QUESTIONS on suspicious content. (2) Secret exfiltration via commits — `open_github_pr.sh` scans staged diff for literal values of `GH_TOKEN` and `GEMINI_API_TOKEN` before committing; aborts if found. (3) Token in clone URL — changed from `https://x-access-token:$TOKEN@...` to plain `https://github.com/...`; `gh auth login` credential helper injects token transparently. (4) Scope violation — post-execution `find` check already in place; `REPO_DIR`-only restriction stated explicitly in both prompts. Full attack surface documented in `SECURITY.md`.
